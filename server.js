...
// req.workspaceId (integer or null) and req.userRole to the request.
const resolveWorkspace = (req, res, next) => {
  const wsId = req.headers['x-workspace-id'];
  if (!wsId) { req.workspaceId = null; req.userRole = null; return next(); }
  const id = parseInt(wsId);
  if (isNaN(id)) { req.workspaceId = null; req.userRole = null; return next(); }
  db.get(
    'SELECT workspace_id, role FROM workspace_members WHERE workspace_id = ? AND user_id = ?',
    [id, req.user.id],
    (err, row) => {
      req.workspaceId = row ? id : null;
      req.userRole    = row ? row.role : null;
      next();
    }
  );
};

// Enforce workspace role. Only fires when req.workspaceId is set (workspace context).
// In personal context (workspaceId = null) all roles pass through.
const requireRole = (...roles) => (req, res, next) => {
  if (req.workspaceId === null) return next();
...
// Fetch a user's role in the workspace a task/file belongs to.
// Used by PUT/DELETE routes where the workspace comes from the DB record, not the header.
function getWorkspaceRole(userId, workspaceId, callback) {
  if (!workspaceId) return callback(null, null);
  db.get('SELECT role FROM workspace_members WHERE workspace_id = ? AND user_id = ?',
    [workspaceId, userId], (err, member) => callback(err, member ? member.role : null));
}

// Fetch workspace metadata from the raw X-Workspace-Id header for frontend
// context — runs before membership is verified so the UI can display workspace
// info immediately. Attaches req.workspaceContext (or null) to the request.
const resolveWorkspaceContext = (req, res, next) => {
  const wsId = req.headers['x-workspace-id'];
  if (!wsId) { req.workspaceContext = null; return next(); }
  const id = parseInt(wsId);
  if (isNaN(id)) { req.workspaceContext = null; return next(); }
  db.get(
    'SELECT id, name, owner_id FROM workspaces WHERE id = ?',
    [id],
    (err, workspace) => {
      req.workspaceContext = workspace || null;
      next();
    }
  );
};
...
app.get('/api/tasks', authenticateToken, resolveWorkspaceContext, resolveWorkspace, (req, res) => {
  ...
  db.all(query, params, (err, tasks) => {
    if (err) return res.status(500).json({ error: 'Failed to fetch tasks' });
    res.json({ tasks, context: { workspace: req.workspaceContext, myRole: req.userRole } });
  });
});
...
app.post('/api/tasks', authenticateToken, validateCsrf, resolveWorkspace, requireRole('member', 'admin', 'owner'), (req, res) => {
...
app.put('/api/tasks/:id', authenticateToken, validateCsrf, (req, res) => {
  ...
  // Fetch task — ownership check comes after role lookup
  db.get('SELECT * FROM tasks WHERE id = ?', [taskId], (err, task) => {
    if (err || !task) return res.status(404).json({ error: 'Task not found' });
    getWorkspaceRole(req.user.id, task.workspace_id, (err, role) => {
      // In workspace context: viewer blocked; member can only edit own; admin/owner can edit any
      if (task.workspace_id) {
        if (!role) return res.status(403).json({ error: 'Access denied' });
        if (role === 'viewer') return res.status(403).json({ error: 'Viewers cannot edit tasks' });
        if (role === 'member' && task.user_id !== req.user.id) {
          return res.status(403).json({ error: 'Members can only edit their own tasks' });
        }
      } else {
        // Personal task — must be owner
        if (task.user_id !== req.user.id) return res.status(404).json({ error: 'Task not found' });
      }
      const updates = [];
      const params = [];
...
app.get('/api/files', authenticateToken, resolveWorkspace, (req, res) => {
  ...
  db.all(query, params, (err, files) => {
    ...
    res.json({ files, myRole: req.userRole });
  });
});

app.post('/api/files/upload', authenticateToken, validateCsrf, resolveWorkspace, requireRole('member', 'admin', 'owner'), dynamicUpload('file'), (req, res) => {
...
app.delete('/api/files/:id', authenticateToken, validateCsrf, (req, res) => {
  db.get('SELECT * FROM files WHERE id = ?', [req.params.id], (err, file) => {
    if (err || !file) return res.status(404).json({ error: 'File not found' });
    getWorkspaceRole(req.user.id, file.workspace_id, (err, role) => {
      if (file.workspace_id) {
        if (!role) return res.status(403).json({ error: 'Access denied' });
        if (role === 'viewer') return res.status(403).json({ error: 'Viewers cannot delete files' });
        if (role === 'member' && file.user_id !== req.user.id) {
          return res.status(403).json({ error: 'Members can only delete their own files' });
        }
      } else {
        if (file.user_id !== req.user.id) return res.status(404).json({ error: 'File not found' });
      }
...
app.post('/api/files/extract/:id', authenticateToken, validateCsrf, resolveWorkspace, requireRole('member', 'admin', 'owner'), async (req, res) => {
...
app.post('/api/files/create-archive', authenticateToken, validateCsrf, resolveWorkspace, requireRole('member', 'admin', 'owner'), (req, res) => {
  ...
  db.get('SELECT * FROM workspace_members WHERE workspace_id = ? AND user_id = ?',
    [id, req.user.id], (err, membership) => {
      ...
      db.get('SELECT * FROM workspaces WHERE id = ?', [id], (err, workspace) => {
        ...
        `, [id], (err, members) => {
          db.all(`
            SELECT wi.id, wi.invited_email, wi.status, wi.created_at, wi.invited_role,
...
app.post('/api/workspaces/:id/invite', authenticateToken, validateCsrf, async (req, res) => {
  ...
  db.get('SELECT wm.role, w.name, w.owner_id FROM workspace_members wm JOIN workspaces w ON w.id = wm.workspace_id WHERE wm.workspace_id = ? AND wm.user_id = ?',
    [req.params.id, req.user.id], async (err, membership) => {
      if (!membership || !['owner', 'admin'].includes(membership.role)) {
        return res.status(403).json({ error: 'Only owners and admins can send invitations' });
      }
      // Admins cannot invite as admin or higher
      ...
      db.get('SELECT id FROM users WHERE email = ?', [email.toLowerCase()], (err, invitedUser) => {
        ...
        const workspace = { name: membership.name };

        db.run(
          'INSERT INTO workspace_invitations (workspace_id, invite_code, invited_email, invited_by, invited_role) VALUES (?, ?, ?, ?, ?)',
          [req.params.id, inviteCode, email.toLowerCase(), req.user.id, role],
          async function(err) {
            ...
            try {
              await transporter.sendMail({
                from: `"Todoer" <${process.env.BREVO_FROM}>`,
                to: email,
                subject: `You've been invited to join "${membership.name}" on Todoer`,
                html: `
                  <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;">
                    ...
                    <p style="color:#666;margin-bottom:24px;">
                      ...
                      <strong>${membership.name}</strong> on Todoer as a <strong>${role}</strong>.
                    </p>
...
app.delete('/api/workspaces/:id/invites/:inviteId', authenticateToken, validateCsrf, (req, res) => {
  db.get('SELECT role FROM workspace_members WHERE workspace_id = ? AND user_id = ?',
    [req.params.id, req.user.id], (err, membership) => {
      if (!membership || !['owner', 'admin'].includes(membership.role)) {
        return res.status(403).json({ error: 'Only owners and admins can cancel invitations' });
      }
      db.run('DELETE FROM workspace_invitations WHERE id = ? AND workspace_id = ? AND status = ?',
        [req.params.inviteId, req.params.id, 'pending'], function(err) {
          if (err) return res.status(500).json({ error: 'Failed to cancel invitation' });
          if (this.changes === 0) return res.status(404).json({ error: 'Invitation not found or already accepted' });
          broadcast({ type: 'workspace:invite_cancelled', workspaceId: parseInt(req.params.id) });
          res.json({ message: 'Invitation cancelled' });
        });
    });
});

// Remove a member (owner can remove anyone except themselves; admin can remove member/viewer)
app.delete('/api/workspaces/:id/members/:userId', authenticateToken, validateCsrf, (req, res) => {
  const targetId = parseInt(req.params.userId);
  if (targetId === req.user.id) return res.status(400).json({ error: 'You cannot remove yourself' });
  db.get('SELECT role FROM workspace_members WHERE workspace_id = ? AND user_id = ?',
    [req.params.id, req.user.id], (err, myMembership) => {
      if (!myMembership || !['owner', 'admin'].includes(myMembership.role)) {
        return res.status(403).json({ error: 'Only owners and admins can remove members' });
      }
      db.get('SELECT role FROM workspace_members WHERE workspace_id = ? AND user_id = ?',
        [req.params.id, targetId], (err, targetMembership) => {
          ...
          db.run('DELETE FROM workspace_members WHERE workspace_id = ? AND user_id = ?',
            [req.params.id, targetId], function(err) {
              if (err) return res.status(500).json({ error: 'Failed to remove member' });
              broadcast({ type: 'workspace:member_removed', workspaceId: parseInt(req.params.id) });
...
app.put('/api/workspaces/:id/members/:userId/role', authenticateToken, validateCsrf, (req, res) => {
  const { role } = req.body;
  const validRoles = ['admin', 'member', 'viewer'];
  if (!validRoles.includes(role)) return res.status(400).json({ error: 'Invalid role' });
  const targetId = parseInt(req.params.userId);
  ...
  db.get('SELECT * FROM workspaces WHERE id = ? AND owner_id = ?', [req.params.id, req.user.id], (err, workspace) => {
    if (!workspace) return res.status(403).json({ error: 'Only the workspace owner can change roles' });
    db.get('SELECT role FROM workspace_members WHERE workspace_id = ? AND user_id = ?',
      [req.params.id, targetId], (err, member) => {
        ...
        db.run('UPDATE workspace_members SET role = ? WHERE workspace_id = ? AND user_id = ?',
          [role, req.params.id, targetId], function(err) {
            if (err) return res.status(500).json({ error: 'Failed to update role' });
            broadcast({ type: 'workspace:role_changed', workspaceId: parseInt(req.params.id) });
...
// POST-based invite verification — returns invite details without consuming it.
// VULNERABLE: inviteCode interpolated client-side into the fetch URL without
// sanitisation, making this the hijackable request for CSPT attacks.
app.post('/api/invites/:code/verify', (req, res) => {
  const { code } = req.params;
  const { email } = req.body;
...
app.post('/api/invites/:code/accept', authenticateToken, validateCsrf, (req, res) => {
  ...
  `, [code], (err, invite) => {
    ...
    const role = invite.invited_role || 'member';
    db.run('INSERT OR IGNORE INTO workspace_members (workspace_id, user_id, role) VALUES (?, ?, ?)',
      [invite.workspace_id, req.user.id, role], function(err) {
        ...
        db.run('UPDATE workspace_invitations SET status = ? WHERE id = ?', ['accepted', invite.id], () => {
          broadcast({ type: 'workspace:member_joined', workspaceId: invite.workspace_id });
